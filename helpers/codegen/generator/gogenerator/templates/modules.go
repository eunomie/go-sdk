...
// isModuleCode, isStandaloneClient, moduleRelPath and boundModule live in
// client.go (extracted with the client path in PR #15); the module generator
// reuses them. Dependencies/HasLocalDependencies are intentionally not ported
// — the standalone client serves a single BoundModule, so nothing calls them.
